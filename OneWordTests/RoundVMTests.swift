...
final class RoundViewModelTests: XCTestCase {
    ...
    func test_playWord_updatesWordInDbAndRemovesFromLocalUser() async throws {
        ...
        let databaseMessages = await database.receivedMessages
        XCTAssertEqual(databaseMessages, [.add])
    }
    
    func test_playWord_throwsIfCantUpdateWordInDatabase() async {
        let (sut, _) = await makeSUT(didAddSuccessfully: false)
        let wordToPlay = sut.localUser.words.first!
        
        await assertDoesThrow(test: {
            try await sut.playWord(wordToPlay)
        }, throws: RoundViewModelError.couldNotPlayWord)
    }
        
    // MARK: Helper Methods
    func makeSUT(didFetchChildRecordsSuccessfully: Bool = true,
                 didAddSuccessfully: Bool = true) async -> (RoundViewModel, DatabaseServiceSpy) {
        let game = Game(groupName: "Test game")
        // fetch one user this way so that a word's user can match one stored in the array
        let userReference: FetchedReference = DatabaseServiceSpy.recordFromDatabase["user"] as! FetchedReference
        ...
        let questionEntry: Entry = DatabaseServiceSpy.recordFromDatabase
        ...
        let database = DatabaseServiceSpy(didAddSuccessfully: didAddSuccessfully, didFetchChildRecordsSuccessfully: didFetchChildRecordsSuccessfully)
